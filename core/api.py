...
def api_transfer():
    ...
    msg = f"{wallet.address}|{receiver}|{'%.16f' % amount}|{nonce}|{ts}"
...
def api_transfer_signed():
    ...
    msg = f"{sender}|{receiver}|{'%.16f' % amount}|{nonce}|{ts}"
...
def api_swap():
    ...
    if pool_bal < amount:
        return jsonify({"error": "兑换池XHB不足(剩余%.4f)" % pool_bal}), 400
    from core.wallet import Wallet
    w = Wallet(pool_priv)
    nonce = int(time.time() * 1000)
    ts = int(time.time())
    msg = "%s|%s|%s|%d|%d" % (pool_addr, to_addr, "%.16f" % amount, nonce, ts)
    sig = w.sign(msg)

...
def init_nft_tables(conn: sqlite3.Connection):
    cur = conn.cursor()
    cur.execute("""CREATE TABLE IF NOT EXISTS nft.nfts (
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS nft.nft_transfer_codes (
...
def mint_nft(conn, minter, name, cid, supply=1, description=''):
    ...
    cur = conn.cursor()
    cur.execute("""INSERT INTO nft.nfts (name, cid, supply, description, minter, owner, created_at)
                   VALUES (?, ?, ?, ?, ?, ?, ?)""",
...
def list_nfts_by_owner(conn, owner):
    init_nft_tables(conn)
    cur = conn.cursor()
    cur.execute("""SELECT * FROM nft.nfts WHERE owner=? ORDER BY id DESC""", (owner,))
...
def get_nft(conn, nft_id):
    init_nft_tables(conn)
    cur = conn.cursor()
    cur.execute("SELECT * FROM nft.nfts WHERE id=?", (nft_id,))
    cols = [d[0] for d in cur.description]
    r = cur.fetchone()
...
def transfer_nft(conn, nft_id, from_addr, to_addr):
    ...
    cur = conn.cursor()
    cur.execute("UPDATE nft.nfts SET owner=? WHERE id=?", (to_addr, nft_id))
    conn.commit()
    return True, nft
...
def create_transfer_code(conn, nft_id, from_addr):
    ...
    # 作废旧码
    cur = conn.cursor()
    cur.execute("UPDATE nft.nft_transfer_codes SET used=1 WHERE nft_id=? AND from_addr=?", (nft_id, from_addr))
    code = secrets.token_hex(16)  # 32位随机码
    ts = int(time.time())
    full_code = f"XiaoHeiBit$Nft${code}${ts}"
    cur.execute("""INSERT INTO nft.nft_transfer_codes (code, nft_id, from_addr, used, created_at)
...
def confirm_transfer(conn, code, to_addr):
    ...
    if len(parts) >= 3 and parts[0] == "XiaoHeiBit" and parts[1] == "Nft":
        code = parts[2]
    cur = conn.cursor()
    cur.execute("SELECT * FROM nft.nft_transfer_codes WHERE code=? AND used=0", (code,))
    cols = [d[0] for d in cur.description]
    r = cur.fetchone()
    ...
    cur.execute("UPDATE nft.nft_transfer_codes SET used=1 WHERE code=?", (code,))
    conn.commit()
    return True, nft
